...
if __name__=="__main__":
	...
	BIDS_root = Path(f"/Users/dominik.welke/Work/02_projects/2023_EEGmanylabs/2023_11-resting-state-pilot/Questionnaire BIDS/{BIDS_folder}")
	
	RAW_folder = Path("/Users/dominik.welke/ownCloud - EEGManyLabs/MetaRep BU2/RestingStateSpinOff")
	...
	for study in ["HajcakHolroyd2005"]:
		...
		eeg_files.sort()
		labs.sort()
		participant_ids_tsv = list(pd.read_csv(BIDS_root / 'participants.tsv', sep='\t').participant_id)
		participant_ids_eeg = []
		for lab in labs:
			print("-"+lab)
			for eeg_file in eeg_files:
				...
				if lab in participant_id:
					...
					try:
						update_eeg_json(BIDS_root, participant_id,task, TaskName="EEGManyLabs Resting State")
		...
		for participant_id in participant_ids_eeg:
			if participant_id not in participant_ids_tsv:
				add_participant(BIDS_root, participant_id, replication=study, lab=participant_id[4:7])
